...
####--->
  ...
  <cfparam name="REQUEST.ctr" default="0" type="integer" />
	...
	<cfthread name="#dynThreadName#" action="run" priority="high">
      ...
      <cfcatch type="any">
      	<!---#### Throw thead-specific exeption to page-thread for handling. ####--->
        <cfthrow message="#cfcatch.Message#" detail="#cfcatch.Detail#" />
      </cfcatch>
    </cftry>
  </cfthread> 

	<!---#### Create a pointer to dynamic thread in the CFTHREAD scope to handle thread variables. ####--->
...
<head>
  ...
  <title>ColdFusion 8: Runaway threads</title>
</head>
